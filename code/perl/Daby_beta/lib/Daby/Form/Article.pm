...
has_field 'tagids'      => ( type => 'Multiple', required => 1, label_column => 'tag', label => 'Tags', size=> 8 );
#has_field 'tagids'      => ( type => 'Multiple', widget => 'CheckboxGroup', required => 1, label_column => 'tag', label => 'Tags');
...
around 'update_model' => sub { 
    ...
    $self->schema->txn_do(sub { 
        ...
        $item->update({ content_summary => $summary, content_location => $location }) if $summary;
    });
};
